...
l=['______step___avgObj_______top1______to5___']
...
def start_train(args):
  ...
  for epoch in range(args.epochs):
    ...
    l.append('train_acc '+ str(train_acc))
    
    df= pd.DataFrame(l)
    df.to_csv('upload/log.txt',header=None,index=False)
    
    # validation
    valid_acc, valid_obj = infer(valid_queue, model, criterion,args)
    logging.info('valid_acc %f', valid_acc)
    l.append('valid_acc '+ str(valid_acc))
...
def train(train_queue, valid_queue, model, architect, criterion, optimizer, lr,args):
  ...
  for step, (input, target) in enumerate(train_queue):
    ...
    if step % args.report_freq == 0:
      ...
      l.append('train %03d %e %f %f'%(step, objs.avg, top1.avg, top5.avg))
      df= pd.DataFrame(l)
      df.to_csv('upload/log.txt',header=None,index=False)
...
def infer(valid_queue, model, criterion,args):
  ...
  for step, (input, target) in enumerate(valid_queue):
    ...
    if step % args.report_freq == 0:
      ...
      l.append('valid %03d %e %f %f'%(step, objs.avg, top1.avg, top5.avg))
      df= pd.DataFrame(l)
      df.to_csv('upload/log.txt',header=None,index=False)
